...
static lv_indev_t *bsp_display_indev_init(lv_display_t *disp)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Touch initialization failed with error: 0x%x", ret);
        ESP_LOGI(TAG, "Continuing without touch functionality...");
        return NULL; // Return NULL as touch is not available
    }
...
lv_display_t *bsp_display_start_with_config(const bsp_display_cfg_t *cfg)
{
    ...
    bsp_display_indev_init(disp); // Don't require touch to be present
